...
export class V1WorkspaceService {
  ...
  async getWorkspaces(query: V1Workspace.WorkspaceQuery = {}): Promise<V1Workspace.WorkspaceResponse> {
    try {
      const {
        limit = 100,
      ...
      const workspaces: V1Workspace.WorkspaceGroup[] = Array.from(workspaceMap.entries())
        .map(([workspace, files]) => ({
          workspace,
      ...
      console.error('Error in V1 workspace search:', error);
      return {
        success: false,
        data: {
          workspaces: [],
