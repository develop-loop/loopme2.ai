...
@ApiTags('V1 Workspace')
@Controller('v1/workspaces')
export class V1WorkspaceController {
  constructor(private readonly v1WorkspaceService: V1WorkspaceService) {}

  @Get()
  @ApiOperation({ 
    summary: 'Get workspaces (V1)', 
    description: 'Find files with workspace metadata in the first 10 lines and group them by workspace. V1 API uses grep -n to find "workspace:" patterns in markdown frontmatter.' 
  })
  ...
  async getWorkspaces(
    ...
    try {
      ...
      return await this.v1WorkspaceService.getWorkspaces(workspaceQuery);

    } catch (error) {
      console.error('Error in V1 workspace controller:', error);
      return {
        success: false,
        data: {
          workspaces: [],
