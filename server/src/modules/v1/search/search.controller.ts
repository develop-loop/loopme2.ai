...
export class V1SearchController {
  constructor(private readonly v1SearchService: V1SearchService) {}

  @Get()
  @ApiOperation({ 
    summary: 'Search files and content (V1)', 
    description: 'Search for files by name or content using find and grep commands. V1 API returns the top 20 most relevant results.' 
  })
  ...
  ): Promise<V1Search.SearchResponse> {
    try {
      ...
      const searchQuery: V1Search.SearchQuery = {
        q: q.trim(),
        type: type || 'both',
        limit: parsedLimit,
      ...
      return await this.v1SearchService.search(searchQuery);
      ...
      console.error('Error in V1 search controller:', error);
